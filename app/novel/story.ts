...
export const STORY: Record<string, Scene> = {
  ...
  choice_escalate: {
    beats: [
      ...
      { kind: 'dialogue', who: 'caller', text: '설명도 없이 그냥 안 된다고요?' },
      { kind: 'dialogue', who: 'protagonist', text: '강설은 자연재해입니다. 저희가 사고 이전에 제설 작업을 완료했고, 판례상 이 경우 지자체 관리하자로 인정되지 않습니다.' },
      { kind: 'dialogue', who: 'caller', text: '그게 무슨 말이에요? 사람이 다쳤으면 책임을 져야 하는 거 아닌가요?' },
  ...
  spring_defend: {
    beats: [
      ...
      { kind: 'dialogue', who: 'caller', text: '그러면 뭘 설치해줄 수 있어요?' },
      ...
      { kind: 'dialogue', who: 'caller', text: '그게 다예요? 담당자가 의지가 없는 거 아닌가요? 옆 단지는 됐다는데.' },
      ...
      { kind: 'dialogue', who: 'caller', text: '이렇게 무책임할 수가 있어요? 국민 세금으로 일하면서.' },
  ...
  spring_yield: {
    beats: [
      ...
      { kind: 'show', chars: [{ id: 'caller', pos: 'caller' }] },
      { kind: 'dialogue', who: 'caller', text: '어, 됐어요? 왜 처음부터 이렇게 안 해줬어요?' },
